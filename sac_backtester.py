# -*- coding: utf-8 -*-
"""SAC_backtester.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1M2yOeG8TE7Tv9t4FDA7NgB4HkUqQNug0
"""
...
from sac import *  # in order to ba able to pickle load the model
...
def add_trade_strategy(df, agent):
  ...
  for i in df.index:
    ...
    # buy one share
    if nn_action > 0.0 and pos < max_pos:
      pos += 1
      cash -= df.loc[i, 'mid']
      action = 1
    
    # sell one share
    elif nn_action < 0.0 and pos > -max_pos:
      pos -= 1
      cash += df.loc[i, 'mid']
      action = -1
...
sac_model = pickle.load( open( '/content/drive/My Drive/DL_Project/sac_strategy/model_obj/sac_201809', 'rb' ) )
BACKTEST_START = '20180901'
BACKTEST_END = '20180930'
PNL_SUMMARY_FILENAME = 'sac_pnl_summary_201809'

OUTPUT_FILE_HEAD = 'sac'

TEST_OUTPUT_DIR = '/content/drive/My Drive/DL_Project/sac_strategy/'
...
sample_end_str_para = '20181231'
stap_length_para = dt.timedelta(seconds=2)
terminal_length_para = dt.timedelta(minutes=1)

env = Input_Processer(sac_model, x_col_names = x_col_names_para, 
...
pnl_summary.to_csv(TEST_OUTPUT_DIR + PNL_SUMMARY_FILENAME + '.csv')
...
for trade_date in pd.date_range('20180101', '20181231'):
  file_dir = '/content/drive/My Drive/DL_Project/sac_strategy/sac_backtest_' + \
            trade_date.strftime('%Y%m%d') + '.csv'
...
turnover_summary.to_csv('/content/drive/My Drive/DL_Project/sac_strategy/turnover_summary.csv')
print('Time took: ', timeit.default_timer() - start_time)

np.mean(turnover_summary['turnover'])